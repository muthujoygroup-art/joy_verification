...
export const exportIndividualCandidateToExcel = (candidate, employerCompany = null) => {
  ...
  const profileRows = [
    ...
    ['Portal Unlock PIN / Passcode', '•••••••• (Encrypted Vault)'],
    ...
    ['Spouse Mobile Number', val(c.spouseMobile || jf.spouseMobile)],
    ['Spouse Occupation', val(c.spouseOccupation || jf.spouseOccupation)],
    ['Children Details', val(Array.isArray(c.children) ? c.children.map(ch => `${ch.name} (${ch.gender || 'Child'}, age ${ch.age || ''} yrs, ${ch.occupation || 'Student'})`).join('; ') : '')],
    ['Father Name', val(c.fatherName || jf.fatherName)],
    ['Father Mobile Number', val(c.fatherMobile || jf.fatherMobile)],
    ['Father Occupation', val(c.fatherOccupation || jf.fatherOccupation)],
    ['Mother Name', val(c.motherName || jf.motherName)],
    ['Mother Mobile Number', val(c.motherMobile || jf.motherMobile)],
    ['Mother Occupation', val(c.motherOccupation || jf.motherOccupation)],
    ['Siblings Details', val(Array.isArray(c.siblings) ? c.siblings.map(s => `${s.name} (${s.relation || 'Sibling'}) - ${s.occupation || ''}${s.mobile ? `, Mob: ${s.mobile}` : ''}`).join('; ') : '')],
    ...
    ['Languages Known', val(Array.isArray(c.languages) ? c.languages.map(l => `${l.name} (${[l.read && 'R', l.write && 'W', l.speak && 'S'].filter(Boolean).join('/')})`).join(', ') : (c.languagesKnown || jf.languagesKnown))],
    ...
    ['Instagram Profile URL', val(c.instagramUrl || jf.instagramUrl)],
    ['Facebook Profile URL', val(c.facebookUrl || jf.facebookUrl)],
    ['YouTube Channel URL', val(c.youtubeUrl || jf.youtubeUrl)],
  ...
  const statutoryRows = [
    ...
    ['State Ration Card Number', val(c.rationCardNo || jf.rationCardNo), 'Recorded', 'Civil Supplies & Consumer Welfare'],
...
export const exportAllCandidatesToExcel = (candidatesList = [], filterTitle = 'Master Workforce Records', companies = []) => {
  ...
  candidatesList.forEach((cand, idx) => {
    ...
    dataRows.push([
      ...
      (c.aadhaarNo || jf.aadhaarNo) ? `XXXX-XXXX-${String(c.aadhaarNo || jf.aadhaarNo).replace(/\D/g, '').slice(-4)}` : '—',
      ...
      (c.bankAccountNo || jf.bankAccountNo || jf.accountNo) ? `••••••••${String(c.bankAccountNo || jf.bankAccountNo || jf.accountNo).replace(/\D/g, '').slice(-4)}` : '—',
