...
const HeroInteractiveCard3D = () => {
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      <div className="flex items-center justify-center p-1 rounded-2xl bg-white/95 border border-slate-200/90 shadow-sm mb-3.5 w-full max-w-[440px]">
        <button
          onClick={() => {
            soundEngine.playClick();
          ...
          className={`flex-1 py-2 px-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
            activeStageMode === 'pass'
              ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-black shadow-md shadow-emerald-600/25 border border-emerald-500'
        ...
        >
          ...
          <span className="text-[9px] bg-emerald-100 text-emerald-950 font-black px-1.5 py-0.5 rounded-full ml-1 animate-pulse border border-emerald-300">3D</span>
        </button>
      </div>

      {/* Role Selection Chips */}
      <div className="flex items-center justify-center gap-2 mb-6 p-1.5 rounded-2xl bg-white/95 border border-slate-200/90 max-w-[440px] w-full shadow-xs">
        {CANDIDATE_PROFILES.map((p, idx) => (
          <button
            ...
            className={`flex-1 py-1.5 px-2.5 rounded-xl text-[11px] sm:text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer truncate ${
              selectedIdx === idx
                ? 'bg-slate-950 text-white font-black shadow-sm border border-slate-800 scale-[1.02]'
                : 'text-slate-600 hover:text-slate-950 hover:bg-slate-100'
            }`}
          >
            <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${selectedIdx === idx ? 'bg-emerald-400' : 'bg-slate-300'}`}></span>
      ...
      {activeStageMode === 'pass' && (
        ...
        >
          ...
          >
            {/* FRONT FACE */}
            <div
              className="absolute inset-0 w-full h-full rounded-3xl dark-glass-card border-2 border-emerald-500/30 hover:border-emerald-500/50 p-6 sm:p-7 flex flex-col justify-between shadow-2xl backdrop-blur-2xl overflow-hidden [backface-visibility:hidden] bg-white/95"
              style={{
                boxShadow: `0 20px 50px -10px rgba(16, 185, 129, 0.18), 0 4px 20px rgba(15, 23, 42, 0.05)`
              }}
            >
              {/* Dynamic Specular Sheen */}
              <div
                className="absolute inset-0 pointer-events-none opacity-25 z-10 transition-opacity"
                style={{
                  background: `radial-gradient(circle at ${tilt.sheenX}% ${tilt.sheenY}%, rgba(167,243,208,0.4) 0%, rgba(52,211,153,0.15) 25%, rgba(6,182,212,0.08) 50%, transparent 75%)`
                }}
              />

              {/* Scanning Laser Beam */}
              {isScanning && (
                <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-transparent via-emerald-400 to-transparent shadow-[0_0_18px_#10b981] z-20 animate-laser-vertical"></div>
              )}
       ...
       * ========================================================================= */}
      {activeStageMode === 'biometric_3d' && (
        <div className="w-full max-w-[440px] h-[520px] rounded-3xl dark-glass-card border-2 border-emerald-500/30 hover:border-emerald-500/50 p-6 flex flex-col justify-between shadow-2xl backdrop-blur-2xl bg-white/95 relative overflow-hidden group">
          {/* Laser Scanning Line */}
          {isScanning && (
          ...
          <div className="flex items-center justify-between border-b border-slate-200 pb-3 relative z-10">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-gradient-to-tr from-emerald-600 to-teal-600 flex items-center justify-center text-white">
          ...
          <div className="relative rounded-2xl overflow-hidden aspect-[4/3] border border-slate-200 bg-slate-950 my-auto shadow-inner group-hover:scale-[1.01] transition-transform duration-500">
            ...
            <div className="absolute top-3 left-3 bg-slate-900/80 backdrop-blur-md px-2.5 py-1 rounded-lg border border-emerald-400/60 font-mono text-[9px] text-emerald-300 font-bold flex items-center gap-1.5 shadow-lg">
      ...
      <div className="flex items-center gap-3 mt-5">
        <button
          onClick={handleTriggerScan}
          disabled={isScanning}
          className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-black flex items-center gap-2 transition-all cursor-pointer shadow-md shadow-emerald-600/30 border border-emerald-500 hover:scale-[1.02] active:scale-[0.98]"
        >
        ...
        {activeStageMode === 'pass' && (
          <button
            onClick={handleFlip}
            className="px-5 py-2.5 rounded-xl bg-white hover:bg-slate-50 border-2 border-slate-900 text-slate-900 text-xs font-black flex items-center gap-2 transition-all cursor-pointer shadow-xs hover:scale-[1.02] active:scale-[0.98]"
          >
