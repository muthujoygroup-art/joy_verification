  ...
  return createPortal((
    <div className="fixed inset-0 z-[9999] overflow-y-auto bg-slate-950/85 backdrop-blur-md p-2 sm:p-4 md:p-6 flex justify-center items-start animate-fadeIn">
      <div className="w-full max-w-7xl bg-white text-slate-900 rounded-3xl shadow-2xl border border-slate-200 animate-modal-spring max-h-[96vh] flex flex-col overflow-hidden">
        ...
        {activeTourMode === 'missions' && (
          <div className="p-4 sm:p-6 overflow-y-auto space-y-6 flex-1 bg-[#F8FAFC]">
            ...
            <div className="bg-white border-2 border-[#426CF5] rounded-3xl p-5 sm:p-7 shadow-xl space-y-6">
              ...
              {/* STEP GUIDANCE CARD */}
              <div className="p-4 sm:p-5 rounded-2xl bg-indigo-50/80 text-slate-900 space-y-3 shadow-xs border-2 border-indigo-200">
                <div className="flex items-center justify-between border-b border-indigo-100 pb-2.5">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="text-[10px] font-mono font-extrabold text-indigo-700 bg-indigo-100 px-2.5 py-0.5 rounded-full border border-indigo-200 hover:bg-indigo-200 transition-colors cursor-pointer flex items-center gap-1"
                    >
                      <Video className="w-3.5 h-3.5 text-indigo-600" />
                      <span>Watch Process Video 🎥</span>
                    </button>
                    <span className="text-[11px] font-mono font-extrabold text-emerald-800 bg-emerald-100 px-2.5 py-0.5 rounded-full border border-emerald-300">
                      INTERACTIVE DEMO
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-1 text-xs">
                  <div className="bg-white p-3.5 rounded-xl border border-indigo-200 shadow-2xs">
                    <span className="font-extrabold text-indigo-900 block mb-1 flex items-center gap-1">
                      🎯 What this feature does:
                    </span>
                    <p className="text-slate-700 leading-relaxed font-sans">{activeStep.whatItDoes}</p>
                  </div>
                  ...
                  <div className="bg-white p-3.5 rounded-xl border border-emerald-200 shadow-2xs">
                    ...
                    <p className="text-slate-700 leading-relaxed font-sans">{activeStep.whyItMatters}</p>
                  </div>
                  ...
                  <div className="bg-white p-3.5 rounded-xl border border-amber-200 shadow-2xs">
                    ...
                    <p className="text-slate-700 leading-relaxed font-sans">{activeStep.nextStep}</p>
                  </div>
                </div>
              </div>
        ...
        {/* ==============================================================================
         * MODE 2: VIDEO TUTORIALS (With YouTube Mini Windows & Video URLs)
         * ============================================================================== */}
        {activeTourMode === 'video_theater' && (
          <div className="p-4 sm:p-6 overflow-y-auto space-y-6 flex-1 bg-[#F8FAFC] text-slate-900">
                ...
                <button
                  ...
                  className={`p-4 rounded-2xl border text-left cursor-pointer transition-all ${
                    activeVideoChannel === ch.id 
                      ? 'bg-white border-[#426CF5] shadow-md ring-2 ring-[#426CF5]/40' 
                      : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-50'
                  }`}
                >
                  <span className="text-[10px] font-mono font-bold text-indigo-700 uppercase tracking-wider block mb-1">
                    {ch.badge}
                  </span>
                  <h4 className="font-bold text-sm font-outfit text-slate-900">{ch.title}</h4>
                  <p className="text-[11px] text-slate-600 mt-1 line-clamp-2">{ch.subtitle}</p>
                  <div className="mt-2 text-[10px] font-mono text-indigo-600 truncate">
                    {ch.videoUrl}
                  </div>
                </button>
              ))}
            ...
            <div className="bg-white rounded-3xl p-6 border-2 border-slate-200 shadow-sm space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <span className="text-xs font-mono text-indigo-700 font-bold uppercase">PROCESS VIDEO TUTORIAL PLAYER</span>
                  <h3 className="text-xl font-bold font-outfit text-slate-900">{activeVideo.title}</h3>
                  <a 
                    href={activeVideo.videoUrl} 
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="text-xs text-indigo-600 font-mono hover:underline inline-flex items-center gap-1 mt-1"
                  >
              ...
              {/* LIVE YOUTUBE MINI WINDOW EMBEDDED PLAYER */}
              <div className="rounded-2xl overflow-hidden border-2 border-slate-300 bg-slate-100 aspect-video shadow-md relative flex items-center justify-center">
                {getYouTubeEmbedUrl(activeVideo.videoUrl) ? (
                  <iframe
                    title={`Process Mini Window: ${activeVideo.title}`}
                  ...
                  <div className="text-center p-8 space-y-2">
                    <Video className="w-10 h-10 text-indigo-500 mx-auto" />
                    <h4 className="text-sm font-bold text-slate-900">Process Tutorial Mini Window</h4>
                    <p className="text-xs text-slate-600 max-w-md mx-auto">{activeVideo.subtitle}</p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                {activeVideo.stats.map((st, i) => (
                  <div key={i} className="p-3 bg-slate-50 rounded-xl border border-slate-200 text-center">
                    <div className="text-[10px] text-slate-500 uppercase font-mono font-bold">{st.label}</div>
                    <div className="text-sm font-bold text-indigo-700 font-mono mt-0.5">{st.value}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* VIDEO TUTORIAL DIALOG MODAL WITH LIVE MINI WINDOW */}
        {activeVideoModal && (
          <div className="fixed inset-0 z-[10000] bg-slate-900/60 backdrop-blur-sm p-4 flex items-center justify-center animate-fadeIn">
            <div className="w-full max-w-2xl bg-white text-slate-900 rounded-3xl p-6 border border-slate-200 space-y-4 shadow-2xl relative">
              <button 
                onClick={() => setActiveVideoModal(null)}
                className="absolute top-4 right-4 p-2 rounded-full bg-slate-100 text-slate-600 hover:text-slate-900 cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-bold text-lg text-slate-900 font-outfit">{activeVideoModal.videoTitle || activeVideoModal.title}</h4>
              </div>

              {/* LIVE YOUTUBE MINI WINDOW PLAYER */}
              <div className="rounded-2xl overflow-hidden border-2 border-slate-300 bg-slate-100 aspect-video shadow-md relative flex items-center justify-center">
                {getYouTubeEmbedUrl(activeVideoModal.videoUrl) ? (
                  <iframe
                    title={`Mini Window: ${activeVideoModal.videoTitle || activeVideoModal.title}`}
                  ...
                  <div className="text-center p-6 text-slate-400 space-y-1">
                    <Video className="w-8 h-8 mx-auto text-slate-400 mb-1" />
                    <p className="text-xs font-bold text-slate-700">No Valid YouTube Link</p>
                    <p className="text-[11px] text-slate-500 font-mono">{activeVideoModal.videoUrl}</p>
                  </div>
                )}
              </div>

              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 text-xs text-slate-700 space-y-2">
                <div className="font-bold text-slate-900">Process Video Summary:</div>
        ...
        {/* MODAL FOOTER */}
        <div className="p-4 bg-slate-100 text-slate-700 text-xs border-t border-slate-200 flex items-center justify-between shrink-0">
