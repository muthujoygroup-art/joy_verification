...
export const FullJoiningFormModal = ({ candidate, isHrMode = false, onClose, onSubmitComplete }) => {
  ...
  const [formData, setFormData] = useState(() => {
    const defaults = {
      ...
      fullName: candidate?.name || jfd.fullName || '',
      empId: candidate?.emp_id || candidate?.empId || jfd.empId || '',
      employeeNumber: candidate?.employee_number || candidate?.employeeNumber || jfd.employeeNumber || '',
      status: candidate?.status || jfd.status || 'Link Sent',
      workingCompany: candidate?.companyName || jfd.workingCompany || 'JOY CORPORATE SOLUTIONS PRIVATE LIMITED',
      ...
      alternateMobile: candidate?.alternateMobile || jfd.alternateMobile || '',
      email: candidate?.email || jfd.email || '',
      state: candidate?.native_state || candidate?.state || jfd.state || '',
      city: candidate?.native_district || candidate?.city || jfd.city || '',
      area: candidate?.area || jfd.area || '',
      pincode: candidate?.pincode || jfd.pincode || '',
      ...
      aadhaarNo: candidate?.aadhaar_no || candidate?.aadhaarNo || jfd.aadhaarNo || '',
      panNo: candidate?.panNo || jfd.panNo || '',
      pfNumber: candidate?.pf_number || candidate?.uanEpf || jfd.pfNumber || jfd.uanEpf || '',
      esiNumber: candidate?.esi_number || candidate?.esicNo || jfd.esiNumber || jfd.esicNo || '',
      drivingLicense: candidate?.drivingLicense || jfd.drivingLicense || '',
      passportNo: candidate?.passportNo || jfd.passportNo || '',
      voterId: candidate?.voterId || jfd.voterId || '',
      uanEpf: candidate?.pf_number || candidate?.uanEpf || jfd.uanEpf || '',
      esicNo: candidate?.esi_number || candidate?.esicNo || jfd.esiNumber || jfd.esicNo || '',
      
      // Detailed EPFO Form 11 & Form 2 Fields
      isPreviousEpfMember: jfd.isPreviousEpfMember || 'No',
      isPreviousEpsMember: jfd.isPreviousEpsMember || 'No',
      previousPfNumber: jfd.previousPfNumber || '',
      prevEmploymentExitDate: jfd.prevEmploymentExitDate || '',
      ...
      esicDispensary: jfd.esicDispensary || '',
      esicBranchOffice: jfd.esicBranchOffice || '',
      esicPrevInsNo: jfd.esicPrevInsNo || '',
      esicPrevEmployerCode: jfd.esicPrevEmployerCode || '',
      factoryEmployerCode: jfd.factoryEmployerCode || '',
      specimenSignature: jfd.specimenSignature || candidate?.specimenSignature || candidate?.specimen_signature || null,

      // Form 16 / TDS Regime Declaration
      taxRegime: jfd.taxRegime || 'New Tax Regime (Section 115BAC)',
      housingLoanInterest: jfd.housingLoanInterest || '0',
      sec80cInvestment: jfd.sec80cInvestment || '0',
      sec80dMediclaim: jfd.sec80dMediclaim || '0',
      otherExemptions: jfd.otherExemptions || '0',

      // Statutory Declarations & Agreement Checkboxes
      agreedForm11: jfd.agreedForm11 !== undefined ? jfd.agreedForm11 : true,
      agreedForm2: jfd.agreedForm2 !== undefined ? jfd.agreedForm2 : true,
      agreedEsicForm1: jfd.agreedEsicForm1 !== undefined ? jfd.agreedEsicForm1 : true,
      agreedForm16: jfd.agreedForm16 !== undefined ? jfd.agreedForm16 : true,
      agreedGratuityFormF: jfd.agreedGratuityFormF !== undefined ? jfd.agreedGratuityFormF : true,
      agreedNda: jfd.agreedNda !== undefined ? jfd.agreedNda : true,
      agreedPosh: jfd.agreedPosh !== undefined ? jfd.agreedPosh : true,
      agreedNonCompete: jfd.agreedNonCompete !== undefined ? jfd.agreedNonCompete : true,
      agreedContractFormXIII: jfd.agreedContractFormXIII !== undefined ? jfd.agreedContractFormXIII : false,
      statutoryFormsConfig: jfd.statutoryFormsConfig || candidate?.statutoryFormsConfig || {},

      // Banking Details
      bankName: candidate?.bankName || jfd.bankName || '',
      accountHolderName: candidate?.name || jfd.accountHolderName || '',
      accountNo: candidate?.accountNo || candidate?.bankAccountNo || jfd.accountNo || jfd.bankAccountNo || '',
      bankAccountNo: candidate?.bankAccountNo || jfd.bankAccountNo || '',
      ifscCode: candidate?.ifscCode || jfd.ifscCode || '',
      branchName: jfd.branchName || jfd.bankBranch || '',
      bankBranch: jfd.bankBranch || '',
      accountType: jfd.accountType || 'Savings Account',

      // Section 4: Education Details (Multi-Entry)
      educationList: jfd.educationList || candidate?.educationList || [],

      // Section 5: Family Member Particulars
      fatherName: candidate?.fatherName || jfd.fatherName || '',
      fatherAge: jfd.fatherAge || '',
      fatherMobile: jfd.fatherMobile || '',
      fatherOccupation: jfd.fatherOccupation || '',
      motherName: candidate?.motherName || jfd.motherName || '',
      motherAge: jfd.motherAge || '',
      motherMobile: jfd.motherMobile || '',
      motherOccupation: jfd.motherOccupation || '',
      spouseName: candidate?.spouseName || jfd.spouseName || '',
      spouseDob: jfd.spouseDob || '',
      spouseDetails: jfd.spouseDetails || '',
      ...
      familyNominees: jfd.familyNominees || [],

      // Section 6: Personal Achievements & Extracurricular Activities
      personalAchievements: jfd.personalAchievements || '',
      extraCurricularActivities: jfd.extraCurricularActivities || '',

      // Section 7: Employment Experience (Multi-Entry)
      experienceList: jfd.experienceList || candidate?.experienceList || [],

      // Section 8: Health, Lifestyle & Background Disclosures (Conditional Yes/No)
      bloodGroup: jfd.bloodGroup || candidate?.bloodGroup || '',
      heightCm: jfd.heightCm || '',
      weightKg: jfd.weightKg || '',
      identificationMark1: jfd.identificationMark1 || '',
      identificationMark2: jfd.identificationMark2 || '',
      hasPreExistingIllness: jfd.hasPreExistingIllness || 'No',
      preExistingDetails: jfd.preExistingDetails || '',
      covidVaccineDoses: jfd.covidVaccineDoses || '',
      ...
      ownsHouse: jfd.ownsHouse || 'No',
      houseCityTown: jfd.houseCityTown || '',
      ...
      // Section 10: Social Media & Online Professional Links
      linkedInUrl: jfd.linkedInUrl || '',
      githubUrl: jfd.githubUrl || '',
      portfolioUrl: jfd.portfolioUrl || '',
      twitterUrl: jfd.twitterUrl || '',

      // Specimen Signature & Document Uploads
      signature: jfd.signature || candidate?.specimenSignature || candidate?.specimen_signature || '',
      specimenSignature: jfd.specimenSignature || jfd.signature || candidate?.specimenSignature || candidate?.specimen_signature || '',
      uploadedDocuments: jfd.uploadedDocuments || candidate?.uploadedDocuments || {},

      // Section 11: Industry Specialization
      industrySpecialization: {
        industryType: candidate?.employeeCategory || jfd.employeeCategory || candSpec.industryType || 'it_tech',
        techStack: candSpec.techStack || candSpec.primaryTechStack || '',
        githubUrl: candSpec.githubUrl || candSpec.gitHubRepoUrl || '',
        portfolioUrl: candSpec.portfolioUrl || '',
        laptopAssetTag: candSpec.laptopAssetTag || '',
        dualEmploymentDisclosure: candSpec.dualEmploymentDisclosure || '',
        plantLocation: candSpec.plantLocation || '',
        shiftRoster: candSpec.shiftRoster || '',
        safetyShoeSize: candSpec.safetyShoeSize || '',
        occupationalHealthCertNo: candSpec.occupationalHealthCertNo || '',
        gatePassId: candSpec.gatePassId || '',
        cibilScoreRange: candSpec.cibilScoreRange || '',
        certificationsBfsi: candSpec.certificationsBfsi || '',
        fidelityBondLimit: candSpec.fidelityBondLimit || '',
        medicalCouncilRegNo: candSpec.medicalCouncilRegNo || '',
        departmentWard: candSpec.departmentWard || '',
        immunizationStatus: candSpec.immunizationStatus || '',
        commercialDlBadgeNo: candSpec.commercialDlBadgeNo || '',
        forkliftLicenseNo: candSpec.forkliftLicenseNo || '',
        policeNocNumber: candSpec.policeNocNumber || '',
        fssaiCertNo: candSpec.fssaiCertNo || '',
        uniformShirtSize: candSpec.uniformShirtSize || '',
        assignedStoreCode: candSpec.assignedStoreCode || '',
        contractFormXIIIEnrollmentNo: candSpec.contractFormXIIIEnrollmentNo || '',
        contractorAgencyName: candSpec.contractorAgencyName || '',
        workOrderPoNumber: candSpec.workOrderPoNumber || ''
      },
