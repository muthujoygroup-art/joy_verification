...
export const LandingPageView = () => {
  ...
  // Client Reviews Dynamic State & API Synchronization
  ...
  const [clientReviews, setClientReviews] = useState(DEFAULT_CLIENT_REVIEWS);

  const fetchPublicReviews = async () => {
    try {
      const res = await api.getPublicReviews();
      if (res && Array.isArray(res) && res.length > 0) {
        setClientReviews(res.map(r => ({
          id: r.id,
          name: r.client_name || r.name || 'Verified Client',
          role: r.designation || r.role || 'HR & Operations Lead',
          company: r.company_name || r.company || 'Enterprise Client',
          stars: r.rating || 5,
          rating: r.rating || 5,
          quote: r.review_text || r.content || r.title || 'Verified Review',
          badge: r.industry === 'labor' ? 'Automotive & Plant' : r.industry === 'logistics' ? 'Logistics & 3PL' : 'Enterprise IT'
        })));
      }
    } catch (err) {
      console.warn('Could not load reviews from API, using default fallback:', err);
    }
  };
